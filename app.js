...
async function init() {
    if (auth.getToken()) {
        ...
        document.getElementById('app-screen').classList.add('flex'); // Mantiene el layout
        await syncData();
        ui.nav('recetario');
    } else {
...
async function api(payload) {
    payload.token = auth.getToken(); // Inyecta el token guardado en la PC/Celular
    ...
    try {
        ...
        // Bloqueo estricto: Si el backend rechaza el token, destruye la sesión
        if (res.error) {
            alert(res.error);
            if (res.error.toLowerCase().includes('denegado')) {
                auth.cerrarSesion();
            }
            return null;
        }
